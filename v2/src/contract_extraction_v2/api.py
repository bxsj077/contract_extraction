...
def create_app(contract_root: Path | None = None, output_root: Path | None = None) -> FastAPI:
    ...
    root = contract_root or Path(os.getenv("CONTRACT_V2_ROOT", str(project_root / "data" / "contracts")))
    output = output_root or Path(os.getenv("CONTRACT_V2_OUTPUT", str(project_root / "data" / "review_output")))
    root.mkdir(parents=True, exist_ok=True)
    output.mkdir(parents=True, exist_ok=True)
    service = ReviewService(root, output)
    app = FastAPI(title="合同解析系统 V2（OCR：PP-OCRv6）", version="2.0.0")
    ...
    @app.get("/api/health")
    def health():
        return {"status": "ok", "version": "V2", "ocr": "RapidOCR / PP-OCRv6",
                "contract_root": str(service.contract_root), "output_root": str(service.output_root)}

    @app.get("/api/config")
    def config():
        return {"合同上传根目录": str(service.contract_root), "审查结果目录": str(service.output_root),
                "项目数据库": str(service.store.path),
                "OCR缓存目录": str(service.ocr_cache_root),
    ...
    def delete_project(project_code: str):
        ...
        cache_root = service.ocr_cache_root
    ...
    def scan(force: bool = Query(False)):
        if not service.contract_root.exists():
            raise HTTPException(400, "CONTRACT_V2_ROOT不存在")
